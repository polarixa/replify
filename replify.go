...
// BodyBytes retrieves the body data as a byte slice from the [wrapper] instance.
//
// This function checks if the [wrapper] instance is available and then attempts to return the body data as a byte slice.
// If the body data is already a byte slice, it returns it directly. Otherwise, it converts the body data to a string and then to a byte slice.
// If the [wrapper] instance is not available, it returns `nil`.
//
// Returns:
//   - A byte slice representation of the body data, or `nil` if the [wrapper] instance is not available.
func (w *wrapper) BodyBytes() []byte {
	if !w.Available() {
		return nil
	}
	if b, ok := w.data.([]byte); ok {
		return b
	}
	return []byte(w.BodyString())
}
